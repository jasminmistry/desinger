<x-app-layout>
    <div class="p-4 sm:p-8 bg-card shadow sm:rounded-lg">
        <div class="flex flex-wrap items-center gap-4 mb-1 border-b border-border ">
            <div class="flex-1">
                <h2 class="text-xl font-medium text-default-700 mb-2">{{ __('Create Digitizing Order') }}</h2>
            </div>
        </div>

        <div class="pt-2">
            <form method="POST" action="{{ route('digitizing.store') }}" enctype='multipart/form-data'>
                @csrf
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="col-span-2 lg:col-span-1 flex flex-col gap-2">
                        ...
                        <div class="flex-1 w-full">
                            <x-text-input id="placement" type="text" name="placement" :value="old('po')" required
                                autofocus />
                    ...
                    <div class="col-span-2 lg:col-span-1 flex flex-col gap-2">
                        <label
                            class="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-50 inline-block mb-2 font-medium text-default-600"
                            for="febric_type">
                            <x-input-label for="febric_type" :value="__('Fabric Type')" />
                        </label>
                        <div class="flex-1 w-full">
                            <x-text-input id="febric_type" type="text" name="febric_type" :value="old('febric_type')" required
                                autofocus />
                            <x-input-error :messages="$errors->get('febric_type')" class="mt-2" />
                        </div>
                    </div>
                    <div class="col-span-2 lg:col-span-1 flex flex-col gap-2">
                        <label
                    ...
                    <div class="col-span-2 lg:col-span-1 flex flex-col gap-2">
                        <label
                            class="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-50 inline-block mb-2 font-medium text-default-600"
                            for="thread_type">
                            <x-input-label for="thread_type" :value="__('Thread Type')" />
                        </label>
                        <div class="flex-1 w-full">
                            <x-text-input id="thread_type" type="text" name="thread_type" :value="old('thread_type')" required
                                autofocus />
                            <x-input-error :messages="$errors->get('thread_type')" class="mt-2" />
                        </div>
                    </div>
                    <div class="col-span-2 lg:col-span-1 flex flex-col gap-2">
                        <label
                            class="text-sm leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-50 inline-block mb-2 font-medium text-default-600"
                            for="background_filled">
                            <x-input-label for="background_filled" :value="__('Background All Filled In')" />
                        </label>
                        <div class="flex-1 w-full">
                            <x-text-input id="background_filled" type="text" name="background_filled"
                                :value="old('background_filled')" required autofocus />
                            <x-input-error :messages="$errors->get('background_filled')" class="mt-2" />
                        </div>
                    </div>
